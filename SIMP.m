function [nelx,nely,nelz,ngrid,stp1,xg,top,lss,fnx,fny,fnz,del_x,del_y,del_z,S] = SIMP(MeshControl,domain,fixed,fixed_x,fixed_y,fixed_z,force_1,...
    force_2,force_3,force_4,force_5,force_6,force_7,force_8,force_9,force_10,keep_domain,...
    keep_BC,keep_BCxyz,vol,E00,nu,penal,rmin,Fmagx,Fmagy,Fmagz,loadtype)
%% Arbitrary Domains
...
[~,~,~,~,edofMatn,nnele,ele,alldofs,~,MusD]...
...
%% Load domain 
U = zeros(ndof,nf);
freedofs = setdiff(alldofs,fixeddof);                                         % make a change here
KE = (aa/0.5)*lk_H8(nu);
...
%% OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
    ...
    for i = 1:size(F,2)
        ...
        c= c+sum(Ee'.*ce);                                                  % make a change here
